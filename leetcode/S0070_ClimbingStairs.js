/*2020-12-28 | 2021-3-23 | 6-8
思路：f(n)有几种，取决于是一步跳过来f(n-1)，还是两步跳过来的f(n-2)，把这两种情况相加，就是到f(n)
错误超时的方法：return climbStair(n-1) + climbStair(n-2) ,导致循环调用，栈溢出
 题目中说了n>=1,所以其实也没必要把给arr中第一个数赋值0
*/
//方法一
