//2020-01-05 |2021-3-7 | 6-3 
//别忘了最开始的边界条件root==null分开讨论，不然后面node.val无意义
// 用队列是因为每一层先存进来的数字要按顺序先输出来，这样可以保证在内循环完这一层之前，已经把下一层的数push进去了，
//但是弹出来的还是之前先push进来的，不会弹后面push的下一层的,所以要看好什么时候push，什么时候shift
// 内层循环的i不一定还在内层用到，只用到shift出来的node就可以了
/**
 * @param {TreeNode} root
 * @return {number[][]}
 */
 var levelOrder = function(root) {
    ...
    while (que.length != 0) {
        ...
        for (var i = 0; i < currentLayerCount; i++) {  //遍历队列长度
            var node = que.shift();   //注意先从队列头部弹出； 所以就用不到i标记
